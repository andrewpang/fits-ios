...
struct PostBookmarkerRowView: View {
    ...
    var body: some View {
        VStack {
            HStack {
                if let profilePicImageUrl = userModel.profilePicImageUrl, !profilePicImageUrl.isEmpty {
                ...
                Text(userModel.displayName ?? "Applauder")
                    .font(Font.custom(Constants.bodyFont, size: 16))
                    .padding(.horizontal, 8)
                Spacer()
                if let bookmarkerUserId = userModel.id {
                    if (authenticationViewModel.isFollowingUser(with: bookmarkerUserId)) {
                        HStack {
                           Text("Following")
                        ...
                        HStack {
                            if (authenticationViewModel.isUserFollowingCurrentUser(with: bookmarkerUserId)) {
                               Text("Follow Back")
                                    .font(Font.custom(Constants.bodyFont, size: Constants.followButtonTextSize))
                                    .padding(.horizontal, 4)
                                    .fixedSize()
                        ...
                        .cornerRadius(10)
                        .onTapGesture {
                            authenticationViewModel.followUser(with: bookmarkerUserId)
            ...
            Button ("Unfollow", role: ButtonRole.destructive) {
                if let bookmarkerUserId = userModel.id {
                    authenticationViewModel.unfollowUser(with: bookmarkerUserId)
                }
            }
