//
//  FollowerFeedWaterfallCollectionView.swift
//  FITs
//
...
struct FollowerFeedWaterfallCollectionView: UIViewControllerRepresentable {
    ...
    func updateUIViewController(_ uiViewController: UICollectionViewController, context: Context) {
        ...
        if (homeViewModel.shouldScrollToTopFollowing && homeViewModel.followingPostsData.postModels?.count ?? 0 > 0) {
            uiViewController.collectionView.scrollToItem(at: IndexPath(row: 0, section: 0),
                                              at: .top,
                                        animated: true)
            homeViewModel.shouldScrollToTopFollowing = false
        }
    }
    ...
    class Coordinator: NSObject, UICollectionViewDelegate, UICollectionViewDataSource, CHTCollectionViewDelegateWaterfallLayout {
        var parent: FollowerFeedWaterfallCollectionView
        var uiCollectionViewController: UICollectionViewController
        var screenWidth = 0.0
        var cardWidth = 0.0
        @Binding var postDetailViewModel: PostDetailViewModel
        
        init(_ parent: FollowerFeedWaterfallCollectionView, uiCollectionViewController: UICollectionViewController, postDetailViewModel: Binding<PostDetailViewModel>) {
            self.parent = parent
        ...
        func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
            if let post = parent.homeViewModel.followingPostsData.postModels?[indexPath.item] {
